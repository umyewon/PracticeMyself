...
public class ContextConfiguration {
	...
	private String driver;			// oracle.dev.driver가 가지고 있는 value값을 필드값(String driver)에 담음
	...
	public BasicDataSource dataSource() {   // 스프링에서 DB연동 모듈 사용하기 위한 객체
		...
		dataSource.setDriverClassName(driver);		// 필드에서 선언한 driver 담기
		dataSource.setUrl(url);						// 필드에서 선언한 url 담기
		dataSource.setUsername(username);			// 필드에서 선언한 username 담기
		dataSource.setPassword(password);			// 필드에서 선언한 password 담기
	...
	public SqlSessionFactory sqlSessionFactory(ApplicationContext context) throws Exception {	// 실행 클래스(Application)에서 선언한 것 사용
		...
		/* mybatis config 파일 설정 : ApplicationContext 객체의 getResource 메소드 사용해서 설정 파일 주입 */
